...
export async function POST(request: Request) {
  try {
    ...
    if (guestFindError) {
      ...
      return NextResponse.json(
        {
          error: "Unable to find guest",
          details: guestFindError.message,
        },
        { status: 500 }
      );
    }
    ...
    const {
      ...
      .from("conversations")
      .select(
        "id, guest_id, guest_name, status, created_at"
      )
    ...
    // 4. Update name on existing conversation
    if (conversation && guestName) {
      ...
      if (nameUpdateError) {
        console.error(
          "Guest name update error:",
          nameUpdateError
        );

        return NextResponse.json(
          {
            error: "Unable to save guest name",
            details: nameUpdateError.message,
          },
          { status: 500 }
        );
      }
